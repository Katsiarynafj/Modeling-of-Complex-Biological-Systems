...
png("bottomly_heatmap_clustered2.png",height=700,width=700)#same as nr1
heatmap.2(edata,
          main = "Bottomly et al. Clustered", # heat map title
          notecol="black",      # change font color of cell labels to black
          density.info="none",  # turns off density plot inside color legend
...
ggplot(PC) + geom_point(aes(x=V3, y=V2, col=as.factor(strain)))
...
p1 <- ggplot(PC) + geom_violin(aes(x=as.factor(strain), y=V1), draw_quantiles = c(0.25, 0.5, 0.75)) + ggtitle("V1")
p2 <- ggplot(PC) + geom_violin(aes(x=as.factor(strain), y=V2), draw_quantiles = c(0.25, 0.5, 0.75)) + ggtitle("V2")
p3 <- ggplot(PC) + geom_violin(aes(x=as.factor(strain), y=V3), draw_quantiles = c(0.25, 0.5, 0.75)) + ggtitle("V3")
p4 <- ggplot(PC) + geom_violin(aes(x=as.factor(strain), y=V4), draw_quantiles = c(0.25, 0.5, 0.75)) + ggtitle("V4")
p5 <- ggplot(PC) + geom_violin(aes(x=as.factor(strain), y=V5), draw_quantiles = c(0.25, 0.5, 0.75)) + ggtitle("V5")

pdf("moj_wykres.pdf", height = 7, width = 7)
...
tsne_out <- Rtsne(edata, pca = FALSE, perplexity = 60)
tsne_out_df <- data.table(tsne_out$Y)

tsne_pdata <- data.table(tsne_out_df, cluster = factor(kmeans_result$cluster), pData(bottomly.eset))

ggplot(tsne_pdata, aes(x = V1, y = V2, color = cluster)) +
  geom_point()
